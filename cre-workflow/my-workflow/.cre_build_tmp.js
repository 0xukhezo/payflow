...
function base64Encode(s) {
  ...
  while (i2 < s.length) {
    ...
    const b = i2 < s.length ? s.charCodeAt(i2++) : 0;
    const c = i2 < s.length ? s.charCodeAt(i2++) : 0;
    out += chars[a >> 2];
    out += chars[(a & 3) << 4 | b >> 4];
    out += i2 - 2 < s.length ? chars[(b & 15) << 2 | c >> 6] : "=";
    out += i2 - 1 < s.length ? chars[c & 63] : "=";
  }
  return out;
}
function hexToBytes2(hex) {
  ...
  for (let i2 = 0;i2 < clean.length; i2 += 2) {
    out[i2 / 2] = parseInt(clean.slice(i2, i2 + 2), 16);
  }
  return out;
}
function hexToBase642(hex) {
  const h = hex.replace("0x", "");
  let binary = "";
  for (let i2 = 0;i2 < h.length; i2 += 2) {
    binary += String.fromCharCode(parseInt(h.slice(i2, i2 + 2), 16));
  }
  return base64Encode(binary);
}
function readChainlinkFeed(runtime2, http, feedAddress) {
...
function assetUsdPrice(asset, ethUsd, btcUsd) {
    ...
    if (validSplits) {
      emp.splits.forEach((split, i2) => {
        units.push({
          ...
          _splitIndex: i2,
          _splitLabel: `split ${i2 + 1}/${emp.splits.length}`
        });
      });
    } else {
      units.push({
...
var onHttpTrigger = (runtime2, payload) => {
  ...
  if (skipped.length > 0) {
    runtime2.log(`[PayFlow] ⚠ Skipping ${skipped.length} unverified: ${skipped.map((e) => e.name).join(", ")}`);
  }
...
┌─ Step 1 · Chainlink Data Feeds (http-actions → Sepolia AggregatorV3) ─┐`);
  runtime2.log("│  CRE nodes independently read on-chain Chainlink prices;");
  runtime2.log("│  DON aggregates via median consensus before proceeding.");
  const ethUsd = readChainlinkFeed(runtime2, http, runtime2.config.feedEthUsd);
  const btcUsd = readChainlinkFeed(runtime2, http, runtime2.config.feedBtcUsd);
  const usdcUsd = 1;
  runtime2.log(`│  ETH  / USD  →  $${ethUsd.toFixed(2)}`);
  runtime2.log(`│  BTC  / USD  →  $${btcUsd.toFixed(2)}`);
  runtime2.log(`│  USDC / USD  →  $1.000000  (stablecoin default)`);
  runtime2.log("└─────────────────────────────────────────────────────────────────────────┘");
  runtime2.log(`
┌─ Step 2 · USDC Peg Check ───────────────────────────────────────────────┐`);
  const toleranceBps = runtime2.config.toleranceBps;
  runtime2.log(`│  USDC peg:   $1.000000 (stablecoin — assumed stable)`);
  runtime2.log(`│  Tolerance:  ${(toleranceBps / 100).toFixed(2)}%`);
  runtime2.log(`│  Status:     ✓ PASS`);
  runtime2.log("└─────────────────────────────────────────────────────────────────────────┘");
  const STABLECOINS = new Set(["usdc", "usdt", "dai"]);
  runtime2.log(`
┌─ Step 3 · Backend Quotes + CRE Rate Attestation ───────────────────────┐`);
  runtime2.log("│  Single backend call fetches all Uniswap/SideShift quotes in parallel;");
  ...
  if (runtime2.config.backendApiUrl) {
    ...
    try {
      const resp = http.sendRequest(runtime2, {
        url: `${runtime2.config.backendApiUrl}/api/payroll/${body.companyId}/cre-quotes`,
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const results = paymentUnits.map((emp) => {
    ...
    if (isStablecoin) {
      ...
      if (bq && bq.settleAmount != null) {
        ...
        if (isSol) {
          runtime2.log(`│    SideShift:   ${settleAmount.toFixed(6)} SOL  [${quoteSource}]`);
          runtime2.log(`│    Deviation:   accepted (no on-chain SOL feed) → CRE ✓ PASS`);
        } else {
          runtime2.log(`│    Uniswap:     ${settleAmount.toFixed(8)} ${asset}  [${quoteSource}]`);
          runtime2.log(`│    Eff. USD:    $${effectiveUsdOut.toFixed(4)} (${(efficiency * 100).toFixed(3)}% of salary)`);
          runtime2.log(`│    Deviation:   ${(deviationBps / 100).toFixed(2)}% (${deviationBps} bps) → CRE ${withinTolerance ? "✓ PASS" : "✗ FAIL"}`);
        }
      } else {
    ...
    runtime2.log(`│    Recipient:   ${effectiveSettleAddress}${emp.splitSettleAddress ? "  (custom wallet)" : ""}`);
    return {
      employeeId: emp.id,
  ...
  const dispatcher = runtime2.config.dispatcherAddress;
...
┌─ Step 5 · On-Chain Report (evm-write → PayrollDispatcher · Sepolia) ───┐`);
    ...
    const tsHex = runtime2.now().getTime().toString(16).padStart(16, "0");
    ...
    runtime2.log(`│  PayrollDispatcher: ${dispatcher}`);
    runtime2.log(`│  PayrollId:         ${payrollId}`);
    runtime2.log(`│  Recipients:        ${recipients.length}`);
    runtime2.log(`│  Total USDC:        ${totalUsdc}`);
    runtime2.log(`│  Status:            ✓ Report submitted to CRE DON`);
    runtime2.log("└─────────────────────────────────────────────────────────────────────────┘");
  }
  let dispatched = false;
  if (queued.length > 0 && failed.length === 0 && runtime2.config.backendApiUrl) {
    runtime2.log(`
┌─ Step 6 · Backend Dispatch ─────────────────────────────────────────────┐`);
    ...
    try {
      const resp = http.sendRequest(runtime2, {
        url: `${runtime2.config.backendApiUrl}/api/payroll/${body.companyId}/run`,
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: base64Encode(safeBody)
      }).result();
