...
function readChainlinkFeed(
  ...
  const body = base64Encode(JSON.stringify({
    jsonrpc: "2.0",
    method:  "eth_call",
    params:  [{ to: feedAddress, data: LATEST_ROUND_DATA }, "latest"],
    id:      1,
  }));
  const resp   = http.sendRequest(
    runtime as unknown as NodeRuntime<unknown>,
    { url: runtime.config.oracleRpc, method: "POST", headers: { "Content-Type": "application/json" }, body },
  ).result();
  const parsed = JSON.parse(bytesToString(resp.body)) as { result?: string; error?: { message?: string } };
  if (parsed.error) throw new Error(`Chainlink feed ${feedAddress}: ${parsed.error.message ?? "RPC error"}`);
  return decodePrice(hexToBytes(parsed.result ?? "0x"));
}
...
function encodePayrollReport(
  ...
  const parts: string[] = [
    ...
    N.toString(16).padStart(64, "0"),
    ...amounts.map(a => Math.round(a * 1e6).toString(16).padStart(64, "0")),
  ];
...
const onHttpTrigger = (
  ...
  if (!body?.companyId)         throw new Error("Missing companyId in request body");
  if (!body?.employees?.length) throw new Error("No employees provided in request body");

  runtime.log("╔══════════════════════════════════════════════════════════════════╗");
  runtime.log("║         PayFlow · Chainlink CRE Payroll Workflow                ║");
  runtime.log("║   Uniswap Trading API · Chainlink Data Feeds · World ID         ║");
  runtime.log("╚══════════════════════════════════════════════════════════════════╝");
  runtime.log(`[PayFlow] Network:        ${runtime.config.networkLabel}`);
  runtime.log(`[PayFlow] Company:        ${body.companyId}`);
  runtime.log(`[PayFlow] Treasury:       ${body.treasury ?? "not provided"}`);
  ...
  if (skipped.length > 0) {
    runtime.log(`[PayFlow] ⚠ Skipping ${skipped.length} unverified: ${skipped.map(e => e.name).join(", ")}`);
  }
  if (eligible.length === 0) throw new Error("No World ID verified employees — payroll aborted");
  ...
  runtime.log(`[PayFlow] Eligible:       ${eligible.length} employee(s) → ${paymentUnits.length} payment unit(s) (total ${totalUsdc} USDC)`);
  ...
  runtime.log("│  Single backend call fetches all Uniswap/SideShift quotes in parallel;");
  ...
  interface QuotesResponse { ethUsd?: number; btcUsd?: number; quotes: BackendQuote[] }
  ...
  if (runtime.config.backendApiUrl) {
    ...
    try {
      ...
      runtime.log(`│  ⚠ Backend quotes failed: ${(err as Error).message} — falling back to oracle prices`);
    }
  }
  ...
  const results = paymentUnits.map((emp) => {
    ...
    runtime.log(`│`);
    runtime.log(`│  ▸ ${emp.name}${splitLabel}  (${emp.salaryUsdc} USDC → ${asset}@${settleChainId})`);
    runtime.log(`│    World ID:    ✓ Verified`);
    runtime.log(`│    Oracle:      $${oraclePrice.toFixed(2)} / ${asset} (Chainlink)`);
    ...
    if (isStablecoin) {
      ...
      runtime.log(`│    Quote:       stablecoin — 1:1 (${settleAmount.toFixed(6)} ${asset})`);
    } else {
      ...
      if (bq?.settleAmount != null) {
        ...
        deviationBps          = isSol ? 0 : Math.round(Math.abs(1.0 - efficiency) * 10000);
        ...
        if (isSol) {
          runtime.log(`│    SideShift:   ${settleAmount.toFixed(6)} SOL  [${quoteSource}]`);
          runtime.log(`│    Deviation:   accepted (no on-chain SOL feed) → CRE ✓ PASS`);
        } else {
          runtime.log(`│    Uniswap:     ${settleAmount.toFixed(8)} ${asset}  [${quoteSource}]`);
          runtime.log(`│    Eff. USD:    $${effectiveUsdOut.toFixed(4)} (${(efficiency * 100).toFixed(3)}% of salary)`);
          runtime.log(`│    Deviation:   ${(deviationBps / 100).toFixed(2)}% (${deviationBps} bps) → CRE ${withinTolerance ? "✓ PASS" : "✗ FAIL"}`);
        }
      } else {
        settleAmount    = oraclePrice > 0 ? emp.salaryUsdc / oraclePrice : emp.salaryUsdc;
        deviationBps    = 0;
        withinTolerance = true;
        quoteSource     = "Chainlink oracle (no route)";
        if (bq?.error) runtime.log(`│    ⚠ Quote error: ${bq.error}`);
        runtime.log(`│    Quote:       no route — oracle fallback (${settleAmount.toFixed(8)} ${asset})`);
      }
    }

    const effectiveSettleAddress = emp.splitSettleAddress || emp.settleAddress;
    runtime.log(`│    Recipient:   ${effectiveSettleAddress}${emp.splitSettleAddress ? "  (custom wallet)" : ""}`);
  ...
  if (dispatcher && dispatcher.length > 2) {
    ...
    const cidHex    = body.companyId.replace(/-/g, "").slice(0, 48).padEnd(48, "0");
  ...
  if (queued.length > 0 && failed.length === 0 && runtime.config.backendApiUrl) {
    runtime.log("\n┌─ Step 6 · Backend Dispatch ─────────────────────────────────────────────┐");
    runtime.log(`│  Endpoint: ${runtime.config.backendApiUrl}/api/payroll/${body.companyId}/run`);

    const dispatchBody = JSON.stringify({ creVerified: true, networkMode: runtime.config.networkLabel.includes("Sepolia") ? "testnet" : "mainnet" });
    ...
    try {
      ...
      const dispatchResult = JSON.parse(bytesToString(resp.body)) as { ok?: boolean; error?: string };
      dispatched = dispatchResult.ok === true;
      runtime.log(`│  Result: ${dispatched ? "✓ Payroll executed by backend" : "✗ " + (dispatchResult.error ?? "unknown error")}`);
    } catch (err) {
  ...
  runtime.log("\n╔══════════════════════════════════════════════════════════════════╗");
  runtime.log(`║  ${queued.length} queued  ·  ${failed.length} failed  ·  ${skipped.length} skipped (unverified)  ·  ${totalUsdc} USDC  ║`);
